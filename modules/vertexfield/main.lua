...
VertexField = {
  ...
  boneTransform=function(arrowindex, bones, arrowx, arrowy, arrowz, data) end,
  receptorTransform=function(arrowindex, bones, arrowx, arrowy, arrowz, data) end,
}
...
function VertexField:DrawArrow(obj, data, i, transform, yoffset) 
  ...
  local yOffset = ArrowEffects.GetYOffset(GAMESTATE:GetPlayerState("PlayerNumber_P"..self.playerNum),column , offBeat)
  local arrowx = ArrowEffects.GetXPos(GAMESTATE:GetPlayerState("PlayerNumber_P"..self.playerNum),column, yOffset)*SCREEN_HEIGHT/480+SCREEN_WIDTH/2
  local arrowy = ArrowEffects.GetYPos(GAMESTATE:GetPlayerState("PlayerNumber_P"..self.playerNum),column, yOffset)*SCREEN_HEIGHT/480+SCREEN_HEIGHT/2+yoffset
  ...
  local rotationz = ArrowEffects.GetRotationZ(GAMESTATE:GetPlayerState("PlayerNumber_P"..self.playerNum),offBeat, false, 100)
  ...
  if structure:GetVisible() then
    for j=0,#self.bonedata do
      structure:SetVertex(j+1,{{bones[(j+1)%#self.bonedata+1]:GetX(),bones[(j+1)%#self.bonedata+1]:GetY(),bones[(j+1)%#self.bonedata+1]:GetZ()},{1,1,1,1},{0,0}})
    end
  end
  ...
  if mesh:GetVisible() then
    for j, triangle in ipairs(self.triangles) do 
      ...
      mesh:SetVertex(j*3-2,{{bones[triangle[1]]:GetX(),bones[triangle[1]]:GetY(),bones[triangle[1]]:GetZ()},{col,col,col,1},{(firstAft:GetX()+tiny*newbonedata[triangle[1]][1])/sw * right,(firstAft:GetY()+tiny*newbonedata[triangle[1]][2])/sh * bottom}})
      mesh:SetVertex(j*3-1,{{bones[triangle[2]]:GetX(),bones[triangle[2]]:GetY(),bones[triangle[2]]:GetZ()},{col,col,col,1},{(firstAft:GetX()+tiny*newbonedata[triangle[2]][1])/sw * right,(firstAft:GetY()+tiny*newbonedata[triangle[2]][2])/sh * bottom}})
      mesh:SetVertex(j*3  ,{{bones[triangle[3]]:GetX(),bones[triangle[3]]:GetY(),bones[triangle[3]]:GetZ()},{col,col,col,1},{(firstAft:GetX()+tiny*newbonedata[triangle[3]][1])/sw * right,(firstAft:GetY()+tiny*newbonedata[triangle[3]][2])/sh * bottom}})
    end
  end
end
